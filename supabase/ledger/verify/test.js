...
const SATS_OVERRIDE = fs.readFileSync(
  path.join(__dirname, '..', '..', 'migrations', '20260826000006_nectar_sats_override.sql'), 'utf8');
...
const PORT = 55433;
// Arbitrary bigint scale for fixture amounts, unrelated to real money — the
// unit is sats after the 2026-08-26 override (20260826000006), and this
// helper only exists so the assertions below stay readable as decimals.
const AMOUNT = (n) => Math.round(n * 1_000_000);
...
async function main() {
  ...
  try {
    await client.query(SATS_OVERRIDE);
    check('sats override applies cleanly', true);
  } catch (e) {
    check('sats override applies cleanly', false, e.message);
    console.log('\nCannot continue.\n');
    process.exit(1);
  }
  ...
  async function post(txn, legs) {
    ...
    try {
      ...
      for (const leg of legs) {
        await client.query(
          `insert into public.ledger_postings (transaction_id, account_id, amount_sats, seed_id)
  ...
  async function balance(accountId) {
    const r = await client.query(
      `select balance_sats from public.ledger_account_balances where account_id=$1`, [accountId]);
    return Number(r.rows[0].balance_sats);
  }

  async function mkPoll(userId, dollars) {
    const inv = await client.query(
      `insert into public.strike_invoices (user_id, requested_amount_sats, strike_invoice_id, api_confirmed_at)
       values ($1,$2,gen_random_uuid(),now()) returning correlation_id`, [userId, AMOUNT(dollars)]);
    ...
    const poll = await client.query(
      `insert into public.strike_invoice_polls
       ...
       values ($1,'PAID',$2,true,'{}'::jsonb) returning id`, [corr, AMOUNT(dollars)]);
  ...
  await post({ kind: 'funding', key: `fund:${f1.corr}`, pollId: f1.pollId },
    [{ account: acct.cash, amount: AMOUNT(10) }, { account: acct.aliceAvail, amount: -AMOUNT(10) }]);
  check('balanced funding commits', await balance(acct.aliceAvail) === -AMOUNT(10));
  check('asset side recorded', await balance(acct.cash) === AMOUNT(10));
  ...
  await expectFail(client, 'unbalanced transaction rejected at COMMIT', () =>
    post({ kind: 'funding', key: `fund:${f2.corr}`, pollId: f2.pollId },
      [{ account: acct.cash, amount: AMOUNT(10) }, { account: acct.aliceAvail, amount: -AMOUNT(9) }]),
    /unbalanced by/);
  ...
  await expectFail(client, 'single-posting transaction rejected', () =>
    post({ kind: 'funding', key: `fund:${f3.corr}`, pollId: f3.pollId },
      [{ account: acct.cash, amount: AMOUNT(10) }]),
    /requires at least 2/);
  ...
  await expectFail(client, 'tip exceeding balance rejected', () =>
    post({ kind: 'tip', key: 'tip:overdraw' },
      [{ account: acct.aliceAvail, amount: AMOUNT(50) }, { account: acct.bobAvail, amount: -AMOUNT(50) }]),
    /overdraft/);

  await post({ kind: 'tip', key: 'tip:1' },
    [{ account: acct.aliceAvail, amount: AMOUNT(3) }, { account: acct.bobAvail, amount: -AMOUNT(3) }]);
  check('tip within balance succeeds', await balance(acct.bobAvail) === -AMOUNT(3));
  check('sender debited', await balance(acct.aliceAvail) === -AMOUNT(7));
  ...
  check('balance unchanged after replay attempt', await balance(acct.aliceAvail) === -AMOUNT(7));
  ...
  await expectFail(client, 'funding without a poll is rejected', () =>
    post({ kind: 'funding', key: 'fund:nopoll' },
      [{ account: acct.cash, amount: AMOUNT(5) }, { account: acct.aliceAvail, amount: -AMOUNT(5) }]),
  ...
  await expectFail(client, 'internal transfer cannot claim external authority', () =>
    post({ kind: 'tip', key: 'tip:fakepoll', pollId: f1.pollId },
      [{ account: acct.aliceAvail, amount: AMOUNT(1) }, { account: acct.bobAvail, amount: -AMOUNT(1) }]),
  ...
  await expectFail(client, 'UPDATE on postings blocked', () =>
    client.query(`update public.ledger_postings set amount_sats = 1 where id = (select min(id) from public.ledger_postings)`),
    /append-only/);
  ...
  await post({ kind: 'seed_plant', key: `seed:plant:${seedA}` },
    [{ account: acct.aliceAvail, amount: AMOUNT(2), seedId: seedA },
     { account: acct.aliceEscrow, amount: -AMOUNT(2), seedId: seedA }]);
  await post({ kind: 'seed_plant', key: `seed:plant:${seedB}` },
    [{ account: acct.aliceAvail, amount: AMOUNT(1), seedId: seedB },
     { account: acct.aliceEscrow, amount: -AMOUNT(1), seedId: seedB }]);

  check('escrow holds both seeds', await balance(acct.aliceEscrow) === -AMOUNT(3));
  check('available reduced by escrow', await balance(acct.aliceAvail) === -AMOUNT(4));

  const perSeed = await client.query(
    `select seed_id, escrowed_sats from public.seed_escrow_balances order by escrowed_sats desc`);
  check('per-seed escrow is provable without per-seed accounts',
    perSeed.rows.length === 2 && Number(perSeed.rows[0].escrowed_sats) === AMOUNT(2),
    JSON.stringify(perSeed.rows));

  await post({ kind: 'seed_bloom', key: `seed:bloom:${seedA}` },
    [{ account: acct.aliceEscrow, amount: AMOUNT(2), seedId: seedA },
     { account: acct.bobAvail, amount: -AMOUNT(2), seedId: seedA }]);
  check('bloom moves escrow to recipient', await balance(acct.bobAvail) === -AMOUNT(5));
  ...
  const seedAfter = await client.query(`select count(*)::int n from public.seed_escrow_balances where escrowed_sats <> 0`);
  ...
  await expectFail(client, 'escrow cannot release more than was committed', () =>
    post({ kind: 'seed_bloom', key: 'seed:bloom:phantom' },
      [{ account: acct.aliceEscrow, amount: AMOUNT(99) }, { account: acct.bobAvail, amount: -AMOUNT(99) }]),
    /overdraft/);
  ...
  const payout = await client.query(
    `insert into public.strike_payouts (user_id, amount_sats, destination_kind, destination)
     values ($1,$2,'bolt11','lnbc...') returning id`, [bob, AMOUNT(5)]);
  const payoutId = payout.rows[0].id;

  await post({ kind: 'withdrawal_reserve', key: `wd:reserve:${payoutId}`, payoutId },
    [{ account: acct.bobAvail, amount: AMOUNT(5) }, { account: acct.pending, amount: -AMOUNT(5) }]);
  check('reserve moves funds out of spendable', await balance(acct.bobAvail) === 0);
  check('reserve parks funds as pending', await balance(acct.pending) === -AMOUNT(5));

  await expectFail(client, 'cannot spend reserved funds', () =>
    post({ kind: 'tip', key: 'tip:afterreserve' },
      [{ account: acct.bobAvail, amount: AMOUNT(1) }, { account: acct.aliceAvail, amount: -AMOUNT(1) }]),
    /overdraft/);

  await expectFail(client, 'withdrawal without a payout record rejected', () =>
    post({ kind: 'withdrawal_reserve', key: 'wd:nopayout' },
      [{ account: acct.aliceAvail, amount: AMOUNT(1) }, { account: acct.pending, amount: -AMOUNT(1) }]),
    /withdrawal_requires_payout/);

  await post({ kind: 'withdrawal_settle', key: `wd:settle:${payoutId}`, payoutId },
    [{ account: acct.pending, amount: AMOUNT(5) }, { account: acct.cash, amount: -AMOUNT(5) }]);
  check('settle clears pending', await balance(acct.pending) === 0);
  check('settle reduces our assets', await balance(acct.cash) === AMOUNT(5));

  // ---- rails mode tripwire ------------------------------------------------
  console.log('\n— tripwire: simulated mode refuses live observations —');

  const modeInv = await client.query(
    `insert into public.strike_invoices (user_id, requested_amount_sats) values ($1,$2) returning correlation_id`,
    [alice, AMOUNT(1)]);
  await expectFail(client, 'live poll rejected while mode=simulated', () =>
    client.query(`insert into public.strike_invoice_polls
      (correlation_id, observed_state, observed_amount_sats, is_simulated, raw_response)
      values ($1,'PAID',$2,false,'{}'::jsonb)`, [modeInv.rows[0].correlation_id, AMOUNT(1)]),
  ...
  await expectFail(client, 'simulated poll rejected once mode=live', () =>
    client.query(`insert into public.strike_invoice_polls
      (correlation_id, observed_state, observed_amount_sats, is_simulated, raw_response)
      values ($1,'PAID',$2,true,'{}'::jsonb)`, [modeInv.rows[0].correlation_id, AMOUNT(1)]),
    /rails mode is live/);
  ...
  const zero = await client.query(`select coalesce(sum(balance_sats),0)::bigint as t from public.ledger_account_balances`);
  ...
  check('assets cover user liabilities',
    Number(s.assets_sats) >= Number(s.user_liabilities_sats),
    `assets=${s.assets_sats} liabilities=${s.user_liabilities_sats}`);

  // ---- the FX scenario: this is the one that bites ------------------------
  console.log('\n— asset write-down: solvency detection when custody is marked down —');

  // NOTE (sats override, 20260826000006): pre-override this scenario modeled
  // BTC price risk — a USD-denominated liability against a BTC-marked-to-USD
  // asset. With sats as the unit of account, our own BTC holdings need no
  // price mark at all (they ARE sats); what's left is the *cash* leg, which
  // is still USD-custodied at Strike and would need marking the other way
  // (USD -> sats) to detect a real shortfall. `fx_mark`/`fx_reserve`'s
  // rationale (design doc) has not been revisited post-override — this test
  // is kept because the invariant it proves (an asset write-down against a
  // fixed liability trips insolvency) is still real and still worth having,
  // not because the BTC-price framing still applies. Flagged, not fixed here.
  const f4 = await mkPoll(bob, 100);
  await post({ kind: 'funding', key: `fund:${f4.corr}`, pollId: f4.pollId },
    [{ account: acct.btc, amount: AMOUNT(100) }, { account: acct.bobAvail, amount: -AMOUNT(100) }]);

  const before = await client.query(`select * from public.ledger_solvency()`);
  check('solvent while the custody mark holds', before.rows[0].is_solvent === true);

  // Write the custody asset down 30 (sats). Our liability to Bob is unchanged.
  await post({ kind: 'fx_mark', key: 'fx:custody-writedown-30', memo: 'custody asset written down 30 sats' },
    [{ account: acct.fx, amount: AMOUNT(30) }, { account: acct.btc, amount: -AMOUNT(30) }]);

  const after = await client.query(`select * from public.ledger_solvency()`);
  check('a custody write-down is detected as INSOLVENCY, not silently absorbed',
    after.rows[0].is_solvent === false,
    JSON.stringify(after.rows[0]));
  check('shortfall is quantified, not just flagged',
    Number(after.rows[0].coverage_sats) === -AMOUNT(30),
    `coverage=${after.rows[0].coverage_sats}`);

  const stillZero = await client.query(`select coalesce(sum(balance_sats),0)::bigint as t from public.ledger_account_balances`);
  ...
  await expectFail(client, 'funding cannot credit a user who did not pay', () =>
    post({ kind: 'funding', key: `fund:${wrongUser.corr}`, pollId: wrongUser.pollId },
      [{ account: acct.cash, amount: AMOUNT(20) }, { account: acct.bobAvail, amount: -AMOUNT(20) }]),
  ...
  await expectFail(client, 'funding cannot credit more than was observed paid', () =>
    post({ kind: 'funding', key: `fund:${wrongAmt.corr}`, pollId: wrongAmt.pollId },
      [{ account: acct.cash, amount: AMOUNT(200) }, { account: acct.aliceAvail, amount: -AMOUNT(200) }]),
  ...
  await post({ kind: 'funding', key: `fund:${feeSplit.corr}`, pollId: feeSplit.pollId },
    [{ account: acct.cash, amount: AMOUNT(10) },
     { account: acct.aliceAvail, amount: -AMOUNT(9.75) },
     { account: acct.fees, amount: -AMOUNT(0.25) }]);
  check('fee split against a funding invoice is allowed',
    await balance(acct.fees) === feesBefore - AMOUNT(0.25));
  check('user credited net of fee',
    await balance(acct.aliceAvail) === aliceBefore - AMOUNT(9.75));
  ...
  await post({ kind: 'funding', key: `fund:${cFund.corr}`, pollId: cFund.pollId },
    [{ account: acct.cash, amount: AMOUNT(10) }, { account: cAcct, amount: -AMOUNT(10) }]);
  ...
  async function spend(cli, key) {
    ...
    await cli.query(
      `insert into public.ledger_postings (transaction_id, account_id, amount_sats) values ($1,$2,$3)`,
      [t.rows[0].id, cAcct, AMOUNT(6)]);
    await cli.query(
      `insert into public.ledger_postings (transaction_id, account_id, amount_sats) values ($1,$2,$3)`,
      [t.rows[0].id, acct.bobAvail, -AMOUNT(6)]);
  }
  ...
  check('exactly one of two concurrent $6 spends applied',
    await balance(cAcct) === -AMOUNT(4), `carol balance = ${await balance(cAcct)}`);
  ...
  // 1 drop = 1 sat, exact, since the sats override (20260826000006) dropped
  // nectar_drop_microusd() — no conversion function left to match against.
  ...
  async function drops(userId) {
    const r = await client.query(
      `select available_sats from public.user_nectar_balances where user_id = $1`, [userId]);
    return r.rows.length ? Number(r.rows[0].available_sats) : 0;
  }
  ...
  const zero2 = await client.query(`select coalesce(sum(balance_sats),0)::bigint as t from public.ledger_account_balances`);
