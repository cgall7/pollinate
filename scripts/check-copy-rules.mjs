...
// the raw-substring arm produces on src/constants/legalCopy.js today. "the
// header sat 12pt lower" was `sats`'s analogue while `sats` was banned
// (2026-09-04 amendment lifted it); it stays as a free negative fixture.
...
// --- C. The rule -------------------------------------------------------
//
// `crypto` carries an exemption the other twelve words do not:
...
// Notes." (`bitcoin` and `sats` shared this shape until the 2026-09-04
// amendment un-banned them outright — their allowlist rows left with them.)
// Same shape as section E's reserved-word allowlist, on purpose, and for the
// same reason stated there — an EXACT-STRING allowlist, not a per-file
// exemption, so a future edit to the one permitted sentence reds here
// instead of silently widening what the whole file is allowed to say. "App
// Review Notes" has no entry because it is an App Store Connect field, not a
// `src/` string this collector ever sees.
const CRYPTO_WORDS_ALLOW = {
  crypto: new Set([
...
// Hoisted above section D's own use so this section's stale-allowlist check
// (below) can share it instead of declaring a second copy.
const texts = new Set(copy.map((c) => c.text));
console.log(`\n--- C. no forbidden word in copy a user reads ---`);
const hits = [];
for (const { word, re } of FORBIDDEN) {
  const allow = CRYPTO_WORDS_ALLOW[word];
  for (const c of copy) {
    if (!re.test(c.text)) continue;
    if (allow?.has(c.text)) continue;
    hits.push(`${word} → ${c.file}:${c.line} [${c.position}] ${JSON.stringify(c.text.slice(0, 90))}`);
  }
}
...
const staleCryptoAllowEntries = [];
for (const word of Object.keys(CRYPTO_WORDS_ALLOW)) {
  for (const s of CRYPTO_WORDS_ALLOW[word]) if (!texts.has(s)) staleCryptoAllowEntries.push(`${word}: ${JSON.stringify(s)}`);
}
check('every crypto allowlist entry is still rendered', staleCryptoAllowEntries, []);
