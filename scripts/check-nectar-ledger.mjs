...
// supabase/migrations/20260826000001_nectar_ledger.sql to an embedded-postgres
// cluster (port 55433 — see the port map in .github/workflows/test.yml) and
// asserts 48 invariants: postings balance to zero, overdrafts are rejected at
// COMMIT, funding requires a matching invoice poll, postings are immutable,
// and the rails_mode guard refuses a real Strike observation while the mode
// is 'simulated'. That last one is the tripwire Sage's 2026-08-26 promotion
// ruling leans on: the migration is authorized in simulated mode only, and
// flipping rails_mode to 'live' stays embargoed on DESIGN.md §9/§13.
//
