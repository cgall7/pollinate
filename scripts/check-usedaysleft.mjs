...
// --- rounding: Math.ceil, matching RotationFrame's original math ---------
...
// --- 0-clamp: past closesAt never goes negative ---------------------------
...
// --- string vs epoch-ms input, both accepted ------------------------------
...
// --- default `now` argument: only assert it doesn't throw / returns a shape
{
  const result = daysUntil(Date.now() + DAY_MS);
  if (typeof result === 'number' && result >= 0) ok('default `now` (no second arg) resolves to a real number');
  else bad('default `now` (no second arg) resolves to a real number', `got ${JSON.stringify(result)}`);
}
