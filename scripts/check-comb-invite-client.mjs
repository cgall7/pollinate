...
check(/navigation\.replace\('Main', \{ screen: 'Today' \}/.test(screen), 'I9 successful join lands on Today, not the obsolete collect route');
check(!/navigation\.replace\(COMB_COLLECT_ROUTE/.test(screen), 'I10 obsolete collect route is no longer in the invite join path');
