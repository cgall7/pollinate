...
> updated `…0007:235`). `ENG-60`'s runtime is `OPS-9`'s `comb-rotation-tick`,
> **merged `182b9f6`/`6d3e54a`** and pending only the prod deploy.
...
| **OPS-9** | Bumble | M | **Rotation scheduler. Landed `182b9f6` (merging `cd734a9`), finisher `6d3e54a` (2026-08-30):** `advance_due_rotations()` (`…0012:155`) seals via `seal_and_send_rotation`, then advances via `comb_advance_rotation` in a **separate** `begin…exception` block (§1B.31.2), scheduled as `comb-rotation-tick` at `*/5 * * * *`. `ENG-60`'s runtime. **Merged, not deployed** — `…0012` is one of the 14 migrations pending on prod | ENG-58 |
...
| **OPS-8** | Lumen + Bumble | S | **Close the analytics contradiction before the privacy policy publishes.** `src/constants/legalCopy.js` — `PRIVACY_POLICY`'s `The short version` body (the analytics sentence) and the first sentence of its `What we do not do` body — promises *"no analytics, crash-reporting or tracking code"* — a published claim that permanently forecloses C1–C4. V2 §20.2 has the fix: **narrow the promise, do not delete it.** **Superseded as a standalone task 2026-09-03 (COPY-9 arbitration):** text source is the arbitrated rewrite (`OUTBOX/COPY9_LEGALCOPY_REWRITE.md`), and it lands in the same commit as `ENG-74`/`ENG-75` per spec §20.4 obligation 1, never as a standalone merge — `POLLINATE_COMB_ROTATION.md` §8.6 row `0.1` carries the ruled wording. Blocks `ENG-89`/`ENG-78` from being honest | — |
...
| ID | Owner | Est | Issue | Deps |
|---|---|---|---|---|
| **COPY-9** | Lumen | M | **Rewrite the two promise sentences** in `legalCopy.js` — `The short version` and `What we do not do`. **Narrow, don't delete:** keep "we never read your entries / never sell or share / no ad networks / no cross-app tracking," retire "zero telemetry." Update the TRIPWIRE block (lines 20–37) in the same edit — it names these exact sentences | ~~Colin: confirm §5A~~ **DISCHARGED** — ruled 2026-08-24, `POLLINATE_V2_SPEC.md:998` at `6f83a6e`. Cell closed 2026-09-03 (Vector). **Amended 2026-09-03 (Vector) — closure upheld, ground narrowed:** the §6 blanket *"nothing in this document is waiting on Colin"* is **withdrawn as a ground for this cell**. The same document contradicts it at **§20.4 obligation 4** (cited by name — this commit inserts lines above it in that file): the four `legalCopy.js` `FILL` placeholders *"still mechanically gate the consent checkbox and still need Colin."* The discharge is **§5A's alone** and does not extend to the legal-copy chain. **Live Colin dependency on this row's commit, narrowed to two values — `LEGAL_ENTITY` and `CONTACT_EMAIL`.** `HOSTING_REGION` is resolved (`ca-central-1`, Bumble 2026-09-03, Supabase Management API) and `EFFECTIVE_DATE` is mechanical; both ride this same commit (Lumen + Bumble). All four read `value: null` at `7e2687f`, so `LEGAL_COPY_READY` is false and no consent checkbox renders; `scripts/check-legal-consent-gate.mjs` arms that transition, so filling the values without re-adding the checkbox goes red. Remaining deps: **`ENG-74` + `ENG-75` in the same commit**; copy is arbitrated in `OUTBOX/COPY9_LEGALCOPY_REWRITE.md` v2 (Lumen, 2026-09-03), which supersedes `OPS-8` |
| **ENG-74** | Pollen | M | Crash reporting (Sentry). Today only the in-app `ErrorBoundary` exists — we cannot currently see a crash at all. **Must land in the same commit as COPY-9.** **Artifact + disqualifier: read `POLLINATE_COMB_ROTATION.md` row `0.1` before cutting.** Copy is hand-cut from the COPY-9 arbitrated rewrite — never from `bumble/ops8-legalcopy-analytics`, whose file predates the ENG-84 storage-model rewrite and would revert it. Two comments misquote the `LEGAL_ENTITY` placeholder as `"[the publisher of this app]"` — `git grep "publisher of this app" -- src/` returns exactly those two sites (the `FILL` header comment in `legalCopy.js`, and the no-checkbox comment inside `Onboarding.js`'s `LegalLinks`). The real value is `FILL.LEGAL_ENTITY.placeholder`. Fix both in this commit; **the grep returning 0 is the acceptance check** | COPY-9 |
...
| ~~**DES-26**~~ | ~~Pixel~~ | ~~M~~ | ~~Paywall surfaces at the delivery moment — must not intrude on the seal/reveal emotion~~ | **FOLDED** per §8's amendment / `POLLINATE_COMB_ROTATION.md` §8.2 — content-lossless: `DES-30`'s row carries the constraint by name (*"must not intrude on a seal or a reveal — same constraint as `DES-26`"*) (@Lumen, 2026-09-03) |
| ~~**COPY-12**~~ | ~~Lumen~~ | ~~S~~ | ~~Pricing + gift copy. Never "upgrade to unlock" language at the reveal~~ | **FOLDED** per §8's amendment / `POLLINATE_COMB_ROTATION.md` §8.2 — clause by clause: the upgrade-to-unlock bar is carried by `DES-30`'s row; the retired $39.99/annual pricing copy was swept by **`COPY-13`** (closed, merged `1ab8e81`) — **do not repoint a Deps cell at it, it is finished**; the residual comb-plan pricing + gift copy cannot be written today (`ENG-79`: *"price unruled — build the plumbing, not the number"*) and **re-arms as a new Lumen row when Colin rules the price** (@Lumen, 2026-09-03) |
