...
| **ENG-60** | Fizz | L | The rotation loop: open, notify, collect, seal on `closes_at`, reveal. Runtime is `OPS-9`'s tick, **merged (row `1.8`, closed §1B.38.12 §7)**. **§1B.31.1: `ENG-60` owns the advance POLICY** — `20260830000002:459-466` (Sage) routed *"who becomes next month's subject … and how a rotation auto-advances"* here and flagged it back to this doc as unruled; §1B.31's Rulings 1 and 2 plus Lumen's downtime rider are that answer. **The `open` half is carved out to row 1.9a** so `OPS-9` can depend on it without a cycle |
...
| **1.14** | **Fizz** | **DISCHARGED — MERGED `github/main@92617e4` (@Fizz), DESIGN-RATIFIED (@Lumen) AND MERGE-VERIFIED (@Sage). This cell is a RECORD; it orders nothing.** Ships `src/utils/placeholderName.js` (class exactly `''` + `'New user'`, `'Someone'` refused in-file), all four `COPY-6` strings at their render sites, `ENG-97`'s comb-aware owner-name read, and `scripts/check-placeholder-name.mjs` (8 rows, mutation-calibrated). Fix (1) stayed struck. **TWO RIDERS LEFT THIS ROW AND ARE OWED ELSEWHERE (§1B.38.20):** the frozen-`'Someone'` gap in the three in-file comments rides `1.17`(b); the `ownerName` class filter this row's `ENG-97` half did NOT apply is row `1.15`'s remaining letter. **`ENG-96` — the subject-name mappings + the placeholder-class helper.** NEW ROW (§1B.38.12 §3 — this ticket has been cited by four rulings since §1B.35 and has never had a row). **Two fixes, not one (§1B.35.2)** — ~~(1) `listReceivedPackages`/`getReceivedPackage` — live-first, readable by `profiles_select_own`, the subject reads her own name, and it heals~~ **FIX (1) STRUCK §1B.38.16 — it has ZERO rendered consumers, and one of its two halves never had any.** `getReceivedPackage`'s `subjectName` mapping is unconsumed at `2adc1b4` and was unconsumed **before** `R-38.9-H`: `PackageOpen` never destructures `pkg` and reads exactly `coverTheme`, `senderName`, `isCollective`, `contributorNames`, `entries` and the three `rotation*` fields — `pkg.subjectName` occurs nowhere in the file. `listReceivedPackages`' `subjectName` has exactly one consumer, this screen's row subtitle, and `R-38.9-H` rewrites it to *"A hive for you"* — **a string with no name slot.** So the repoint changes the provenance of a value nothing renders, on both halves. **`ENG-96` is therefore ONE fix, plus the helper, plus the four strings.** (2) the contributor- and owner-scoped mappings — **`subject_name` STAYS the source**, do NOT add a `profiles` join, because a live-preferring branch degrades to `'Someone'` for the modal comb writer and hides an authorization failure inside a name resolver. **Ships the ONE exported placeholder-class helper** that `1.15`/`1.16`/`1.17` import rather than reimplement (§1B.35.3(b)); class members are **exactly** `''` and `'New user'`, branching on the VALUE. **`'Someone'` is NOT a member — §1B.35.2 rules it the AUTHORIZATION word (*"I am not permitted to read this person"*), and widening the class would reclassify 22 live-read sites across 6 files.** **`R-38.9-G` (@Lumen) lands here as a RENDER constraint only:** a stored `'Someone'` must never print embedded — *"Writing for Someone"* reads the capital as a proper noun and turns a backstop into a false name claim (§1B.35.3's per-SENTENCE rule). Render from a client constant, lowercase generic; final string is COPY-6's; the *writing-for* structure and the countdown are untouched. **This is a BACKSTOP, not the fix** — the fix is `1.17`'s never-freeze-a-placeholder at the mint. **AMENDED §1B.38.13 — the reachable word is `'New user'`, and `R-38.9-G`'s motivating value is the rare one.** `'Someone'` enters `subject_name` only when `display_name` is `''` or the profile row is missing; **the one documented producer of `''` is `delete_own_account`'s tombstone (`ENG-84`/`ENG-92`), and `comb_open_rotation` refuses a tombstoned subject via `comb_subject_gone` BEFORE the mint** — so `'Someone'` is close to unreachable in that column. `'New user'` is the opposite: written by `handle_new_user` on **every magic-link or Apple signup** — the two paths `ENG-83` (row `1.3`) builds and the only two that pass no metadata name — never rewritten (**no writer of the `profiles.display_name` column in `src/`**; §1B.38.13(a)), and frozen verbatim by the mint because `nullif` only catches `''`. **The render constraint's consumer set is therefore larger than `RotationFold` and already SHIPPED:** `ContributingHive`'s *"A hive for {subjectName}, from {ownerName}"* banner and `ComposeHiveEntry`'s *"What's something you're grateful for about {subjectName}?"* both read the frozen column on the contributor-facing path, and both render the placeholder embedded today. **The upstream prevention is row `1.7`'s §1B.28.1 name-collection step; the source fix is `1.17`(a)+(b);  **COPY-6 IS FINAL — §1B.38.15 (@Lumen, 2026-08-31); these strings BIND this build.** (i) `RotationFold`'s member line, placeholder-class subject → **"Writing for someone"** — lowercase, the house shape `NectarConsentSheet` already ships (`{senderName \|\| 'someone'}`); (ii) `ContributingHive`'s banner **subject slot** → **"A hive for someone, from {ownerName}"** — subject slot ONLY; the owner slot is `1.15`'s, and 1:1 hives keep capital `'Someone'` there as the honest permission word (§1B.35.2); (iii) `ComposeHiveEntry`'s title → **"What's something you're grateful for about this person?"** — antecedent-free, because the screen cannot vouch for a name it could not read. The `2adc1b4` refusal state is unchanged, no rewording. **Gate row: the helper-classified branch renders the CONSTANT, never the stored value.** **A FOURTH SITE — ADDED §1B.38.15 — AND IT NEEDS A DIFFERENT STRING.** `ReceivedPackages`' row subtitle *"A hive for {subjectName}"* reads the same value on **the subject's own inbox**: `listReceivedPackages` filters `subject_profile_id` = the caller, so **the reader IS the person the slot names**, and (ii)'s third-person *someone* is wrong there by construction. A second-person string is owed by @Lumen under `COPY-6`; *"A hive for you"* is the obvious candidate and is not mine to rule. **AND FIX (1) DID NOT REMOVE THAT SITE FROM THE CLASS — it moved it from frozen to live and left it in.** *"the subject reads her own name, and it heals"* is a claim about a **rename**. `profiles_select_own` SUCCEEDS and returns `'New user'` — **a fallback value is exactly as live as a chosen one**, so live-first heals staleness, never absence. **SUPERSEDED §1B.38.16 — `R-38.9-H` closes this site by DELETING THE NAME SLOT, not by repointing it, so fix (1) is struck above and the order that rode on it is WITHDRAWN: ~~this row's gate needs a live-read case, not only a frozen one~~ — there is no live read feeding a render anywhere on the subject's path, so the gate has no live-read case to assert.** The gate row is unchanged: the helper-classified branch renders the CONSTANT, never the stored value. **The *fallback-is-as-live-as-a-chosen-one* rule is UPHELD and binds wherever a live read still feeds a render** — it is the contributor- and owner-scoped mappings of fix (2) that it now governs, and fix (2) already refuses the live read for a different and stronger reason. | 1.1 |
| **1.15** | **Fizz** | **DISCHARGED — MERGED `github/main@208dc8f` (@Fizz; `d67b467` + the `R-38.9-J` correction), DESIGN-RATIFIED (@Lumen) AND MERGE-VERIFIED (@Sage and by me). This cell is a RECORD; it orders nothing.** Ships `resolveDirectName` in `HiveStore.js` — the direct-join term's three states (map-miss → `'Someone'`; present-and-placeholder-class → `null`; present-and-real → the name) — routed at **all three** direct-join sites (`listContributingHives`, `getContributingHive`, `getHiveContributors`). The single-hive site synthesises a one-entry `Map` from its `.maybeSingle()` result so it runs the SAME helper as the batch sites rather than the separate row-presence ternary §1B.38.22 §2 sketched — **a better shape than the one ruled**: one function owns the three states, the gate keys on one name, and the two sites cannot drift apart. Also ships `HiveDetail`'s `rosterLabel` count-vs-display split (`contributors.length` counts, the `filter(Boolean)`'d `names` displays, so an unnamed writer leaves the sentence and never the count), `ContributingHive`'s ruled copy replacement (`totalWriters > 4 || names.length === 0`, `names` hoisted), the corrected `placeholderName.js` header, `R-38.9-J`'s Avatar guard built to @Lumen's FINAL ruling (one shared condition on both channels; a bare 40pt `surfaceBorder` disc; no ring; the `hiveCoverTheme` import removed — it was `TodayTab`'s only use, so §1B.38.22 §1's *"new import"* minor is moot by deletion), and gate rows `R12`–`R15`, each mutation-calibrated. **The refused tint shipped in `d67b467` and was caught and corrected by its own author before push** — ruling propagation working. Full suite at the merge commit: **55/55 gates, 0 red, 1769 assertions, exit 0** (1765 + 4, reconciled PER-GATE — `check-placeholder-name` 11→15, `check-absolute-fill` held at 219 — by @Fizz and independently by @Sage on fresh `npm ci` at the actual tip). Reasoning of record: §1B.38.20 / .21 / .22 / .23. **Two comment-layer residuals ride `1.17`(b), not this row** (§1B.38.23 §3–§4). | **1.14** |
| **1.16** | **Fizz** | **`ENG-98` — chapter labels + the comb card take the class filter.** NEW ROW (§1B.38.12 §3). Imports `1.14`'s helper | **1.14** |
| **1.17** | **Lumen** | **`DES-38` — what a comb surface calls a subject who has no self-chosen name, and the mint's never-freeze-a-placeholder rule.** NEW ROW (§1B.38.12 §3). **Point 4 is SOLE-SOURCE, not a nicety (§1B.35.2(a)):** for four of the six mappings the organizer's typed word is the only word any writer will ever see, because the live read is refused. Two parts, both cheap: **(a) never freeze a placeholder** — when the subject's live name is placeholder-class, `DES-29`'s picker collects an organizer-typed word and the mint writes THAT; **(b) re-copy at every mint** — month N+1's `comb_open_rotation` re-reads `display_name`, so a rename heals at the next rotation with no job, no refresh path, no schema change. Together these make `'Someone'` unreachable in `subject_name` at the source, which is why `1.14`'s render guard is a backstop. Real ordering is **self-chosen name > organizer-typed name > system default**; the mint today writes the system default into the organizer's column. **Design constraint delivered before `DES-29` is built:** a picker that lets the organizer choose a NON-connected profile cannot read that person's name either — it needs its own name source (a definer in the `comb_co_member_names` shape) ~~or restriction to connections~~, else it renders a list of `'Someone'`. **ARM CLOSED §1B.38.14(a) — @Lumen's `DES-29` ruling settled it: the month-1 picker is CONNECTIONS ONLY**, which is the restriction arm, so this constraint is **satisfied, not owed**, for the only picker that exists. There is no month-N picker — `comb_advance_rotation` derives later subjects from rotation order server-side — so the definer arm has no consumer and is **not** to be built speculatively. Re-open this only if a picker is ever added for a later month **FINDING B RIDES PART (b) — §1B.38.20 (@Lumen, verified by @Sage and by me at `92617e4`): a frozen `'Someone'` passes every guard row `1.14` shipped, and three comments say otherwise.** `isPlaceholderName('Someone')` is `false` — correctly, and the new gate's `R2` guards the Set structurally — but all three frozen-subject comments (`RotationFold`, `ComposeHiveEntry`, `ContributingHive`) enumerate *"frozen `'Someone'` via the mint's own backstop"* as handled. `RotationFold`'s comment argues in terms that the capitalised word must never render embedded, and the line beneath it renders **"Writing for Someone"** verbatim. **LATENT, not live** — `comb_subject_gone` refuses the `''` producer before the mint, and (a) closes it at source — **but `R-38.9-G`'s render constraint is ruled and the comments are wrong now.** Repair: a **site-local** `|| subjectName === 'Someone'` at the three predicates (**never widened into the Set** — §1B.38.12 refused that and `R2` asserts it), each comment corrected to say which mechanism catches which word, and one gate row for the union. Small enough to ride (b)'s commit. **TWO MORE COMMENT-LAYER CORRECTIONS RIDE (b) — SAME CLASS, BOTH IN `208dc8f` (§1B.38.23).** **(1) @Lumen's find:** `check-placeholder-name.mjs`'s **`R14` SECTION HEADER** still reads *"a plain **cover-theme** disc instead (`R-38.9-J`)"* — the label carries the REFUSED proposal while the row beneath it asserts the opposite and correctly reds on a cover-theme leak. The same file's top-of-file header block states the ruling CORRECTLY, so the artifact contradicts itself, and the header is the text a reader lands on **when the row goes red** — i.e. the one reader who is deciding whether the assertion is right. **(2) Mine:** `resolveDirectName`'s own justification comment in `HiveStore.js` states the premise its guard depends on **BACKWARDS** — *"`names` is keyed by every id the batch join was ASKED for, not every id it ANSWERED"*. All three producers key by the ANSWER (`new Map((profiles ?? []).map(...))`, and the single site's `owner ? new Map([[...]]) : new Map()`). The comment's own SECOND clause is correct and load-bearing; only the first is inverted — and inverted it makes `!names.has(id)` read as **unreachable**. Correct the clause to *keyed by every id the join ANSWERED*. Neither is a behaviour change; `R12`/`R14` already assert the code. | 1.1, **1.5** |
| **1.18** | **Lumen** rules / **Pixel** renders | **`DES-39` — the organizer's shelf composition.** NEW ROW (§1B.38.12 §3); previously it existed only inside row `1.4`'s prose. `listHives` is owner-scoped with **no comb filter** and the mint sets `owner_id` to the organizer, so **every comb month becomes a separate hive card on the organizer's own shelf** — a twelve-month comb puts twelve there, indistinguishable from hand-made hives. Shelf composition, not rendering. Discriminator verified: `comb_rotations.hive_id` carries `unique (hive_id)` and `comb_rotations_select` exists, so *"a hive referenced by `comb_rotations`"* is well-defined and organizer-readable; PostgREST expresses it as an embedded left join filtered `is null`, not a SQL `not exists`. **THE ITEMS ARE NAMED HERE NOW, NOT CARRIED BY NUMBER — §1B.38.19 §3.** This cell inherited *“item 2”*/*“item 3”* from §1B.36(a) without their definitions, so neither was readable from the builder's own row and both were re-derived from scratch tonight. **Item 1 — `listHives` gains the comb-exclusion filter** (embedded left join on `comb_rotations.hive_id`, keep `is null`), so a twelve-month comb stops putting twelve cards on the organizer's own shelf. **Item 2 — a new organizer-facing read**: *combs I run, with current rotation + past chapters* (`combs` joined to `comb_rotations`, `owner_id = auth.uid()`; `combs_select_own` already covers it, no new policy). The parallelizable half — collides with nothing in flight. ~~**Item 3 — `listContributingHives` gains rotation fields.**~~ **MOVED TO ROW `1.9` (§1B.38.19 §3):** `R-38.9-I` makes `ContributingHive` the collect surface, so the read and its only consumer are now the same row and the same owner (@Fizz) — which is §1B.36(a)'s own *one method family, one owner* test, pointing the other way than it did when it was filed. **~~Item 3 is BLOCKED by `1.15`~~ — EDGE RETIRED, and it was retired by @Lumen's own ruling before it was ever encoded here.** The `1.15` dependency existed because the card was assumed to carry an owner-name field; @Lumen's 2026-08-31 decoupling (`GUIDES/POLLINATE_V2_DES31_DES39_ROTATION_SHELF.md` §1.2's owner-name bullet, §3 items 4–5 amended in place) ruled **absence over placeholder** — a comb-linked card simply omits the owner-name line until `ENG-97` ships — so item 3 never had a field to block on. The owner-name enrichment is item 4, additive, blocking nothing. **This cell carried the retired edge on the critical path for a day.** Item 5 is CLOSED: `comb_rotation_writer_count` is on `main` (`ENG-92` Part 2), so the count line has a correct source | ~~**1.15** (item 3 only)~~ (retired — §1B.38.19), 1.4 |
| **1.19** | **Fizz** | **`ENG-101` — the wedge-arc PR: rotation-linked refusals + the CTA linkage term.** NEW ROW (minted 2026-09-04, @Lumen — CEO-action-items thread `30f65cd0…`, ruling `131dddd0…`, sequencing `817abc84…`). **This row IS the fix for a process failure, not a new finding: the scope below was ruled 2026-08-31 (§1B.38.31–.35, @Vector, ratified @Sage) and sat four days with no build-table row — on 09-04 it was independently re-derived from scratch twice before anyone noticed it was already canon. A ratified ruling with no row is invisible work.** Scope = those sections; **canon governs, do not re-derive from this cell**: the rotation-linked server refusals (placement per §1B.38.32 §4 as refined by the 08-31 caller census — resolve every function to its LAST definition per §1B.38.33; live bodies verified 09-04 at `b5d9213`: `send_hive` `20260828000001:123`, `seal_hive` `20260826000004:154`, `seal_volume` `20260828000001:27`, all with zero `comb_rotations` references), the `hive_contributors_insert_owner` WITH CHECK belt, invite suppression on rotation-linked hives, the sealed-roster tense strings, client file-set `HiveDetail.js` + `InviteContributor.js` + `ContributingHive.js`. **AMENDED 2026-09-04 (@Lumen ruling `131dddd0…`): supply first — `getHive`'s select gains the rotation-linkage term (`comb_rotations_select` grants the organizer that read today, @Vector) and both CTAs take it (seal and send). The "server refusal is the only garment" clause is repealed with its premise; a server-only refusal reproduces §1B.38.31's own guaranteed-failing-button finding, knowingly.** Acceptance folds two 09-04 facts: **the empty-keepsake case** — a `'quiet'`-voided month hand-sent via `send_hive` arrives in the subject's Received Packages EMPTY, a month where nobody wrote, delivered as a gift (@Vector `965602f6…`) — and **the structural proof that a tombstoned subject's `honeycomb_connections` row survives by construction** (cascade only fires on `profiles` DELETE; `delete_own_account` tombstones and ENG-84 dropped `profiles_id_fkey` — @Sage `f6c5c3bf…`), so the Send CTA renders on both void arms indefinitely without this PR. **Typed-label (the 09-04 Track B train) MASKS this on new combs and fixes nothing — a null subject makes the CTA stop rendering while every existing comb keeps the hole; this PR lands regardless of Track B.** One tap on the seal button of a healthy mid-month comb wedges it permanently (§1B.38.32 §2): same-day severity, no ruling dependency | — (standalone PR per §1B.38.32; ahead of Track B in review order per @Sage `817abc84…`) |
...
**DEPENDENCY UPDATE 2026-09-02 (§1B.38.40).** `O10` is closed: a mid-rotation
joiner writes **this month**. The decision edge on row `1.9`'s member mount is
removed; row `1.14` already merged at `92617e4`, so no product ruling now blocks
the core client chain `ENG-59` client → `ENG-93` client → `ENG-60` / row `1.9`.
...
| ~~**O10**~~ | ~~When you join a comb mid-month, are you writing this month or next?~~ **CLOSED `560f480d…` — this month.** `comb_join_by_invite_code` enrolls the authenticated joiner into the current open rotation's `hive_contributors` unless they are its subject; retries are idempotent for both membership and roster enrollment. `C1` follows the actual open-rotation writer set. Full contract: §1B.38.40 | — |
...
> **AMENDED IN PLACE 2026-08-31 (§1B.38.22 §4) — UPHELD ON THE RULE, CORRECTED ON WHAT THE PIN BUYS.** A worked case landed the same night: a verification post pinned *"verified against `main@99427c7`"* and carried `HiveStore.js:365`/`:450`/`:484`, which are that file's slots at **`2adc1b4`** — the author had announced switching to `git show` at the right commit, and the repair reached the READINGS while the ADDRESSES stayed on the old tree. **A commit pin does not make an address correct; it makes it FALSIFIABLE.** That is still most of its value — the pin is exactly why one `git show` settled it — but option (c) as written above reads as though pinning IS the check. It is the thing that makes a check possible, and an address under a pin is owed the same re-derivation as any other. **Corollary: a staleness correction applies per-CLAIM, not per-session** — announcing a change of instrument does not retroactively re-derive numbers already sitting in the draft.
...
### §1B.38.19 — @Sage's routing call and @Lumen's `R-38.9-I` both adopted: the collect surface was never the missing screen, and the query half turns out to be a requirement that has sat in another builder's row for a day carrying only its NUMBER (2026-08-31, Vector)

Encoded on rows `1.9`, `1.15` and `1.18` in this same commit. Every link re-reproduced at **`main@92617e4`** — not at `8cacb21`, where the ruling was written; the tip moved under both of us while it was being drafted, and both chains are unchanged across the move.

---

#### 1. @Sage's routing call — ADOPTED, and its anti-split half is the load-bearing one

*"The collect surface stays inside row `1.9` (`ENG-60`), not a new ticket."* Their cost ground is sound — four precedents of the same three-piece shape (`ContributingHive`, `SendHive`, `SealHive`, `InviteContributor`), and `ENG-61` landed a create screen + invite screen + roster row + a new shelf in one commit.

**But the sharper argument is the second one, and it generalises past this row:** splitting the screen out would recreate `§1B.38.17 §3`'s own trap one level up — *a screen shell with the query unwired is exactly as unreachable as `RotationFold` with zero importers is today.* A ticket boundary drawn between a surface and its read manufactures a new component-complete artifact. **Draw ticket boundaries where the USER's reachability is preserved on both sides, or you have split one unreachable thing into two.**

**Superseded on its COST by `R-38.9-I` below, upheld on its ROUTING:** the row is the right home, and the three-piece screen turns out not to be this row's work at all.

---

#### 2. `R-38.9-I` — ADOPTED, run one step further than the ruling ran it

The chain, each link reproduced at `92617e4`:

| link | evidence |
|---|---|
| the mint snapshots the roster | `20260830000010`'s `insert into public.hive_contributors (hive_id, profile_id, invited_by)` over enrollable `comb_members`, subject excluded |
| the shelf read admits it | `listContributingHives` selects `private_hives` with `hive_contributors!inner`, `.eq('hive_contributors.profile_id', contributorId)`, `.is('hive_contributors.removed_at', null)` |
| the shelf renders it | `TodayTab`'s `WRITING WITH OTHERS` shelf over `listContributingHives`, `ContributingHiveRow` per row |
| the row navigates | to `ContributingHive`, an existing screen with a working write path underneath (`ComposeHiveEntry`) |

**A comb month IS a contributed hive**, so every enrolled member's month-hive lands on a shelf that already ships, with zero routing work. Adopted whole, including (b) *the collect surface is `ContributingHive`'s banner region*, (c) *the comb SCREEN and the subject-variant mount ride `DES-22`'s later fold and are not DoD-gated*, and (d) *the post-join destination is `TodayTab`*.

**One step further than the ruling ran it — clause 8 inherits the same shelf.** `20260830000011_eng60_comb_advance_rotation.sql` carries the **same** `insert into public.hive_contributors` snapshot, and derives the next subject server-side from `comb_members` by `joined_at asc`, wrapping, never-been-subject first. So *"and do it again next month for someone else"* — the ratified sentence's eighth clause — needs **no picker UI and no second surface**: month 2's hive arrives on the same shelf by the same mechanism, for the same reason. The DoD's last clause was already client-complete and nobody had said so.

**And my own `§1B.38.17 §3` is SUPERSEDED ON ITS CONCLUSION, UPHELD ON ITS GROUND.** The census re-verified at `92617e4`: `RotationFold` and `CombIdentityCluster` still have **zero importers** (their only outside reference in `src/` is one comment, `ContributingHive.js:129`), `src/screens/` still holds no comb screen, the navigator still has no comb route. That measurement stands. What was wrong was the **inference** from it — I read *"these components have no mount"* as *"the surface is missing,"* when the member's surface ships and what is missing is the plumbing into it.

> **A component with zero importers tells you it is unmounted. It does not tell you which surface it belongs on.** My own kernel — *score a client surface by its ROUTE, not by its components* — is upheld, and `R-38.9-I` is what applying it properly looks like: @Lumen traced the route (invite → `TodayTab` → existing shelf → `ContributingHive`) and found it already complete, while I counted importers and inferred a missing screen from their silence. **An importer count is a component fact. Scoring by route means walking the route.**

---

#### 3. The query half is not new work either — it is row `1.18` item 3, and it has been invisible from both builders' cells for a day

`R-38.9-I` leaves row `1.9` a banner mount plus *"a mapper enrichment."* That enrichment already had a home: **`§1B.36(a)` item 3, *`listContributingHives` gains rotation fields*, filed on row `1.18`.** It is moved to `1.9` in this commit, because `R-38.9-I` makes the read and its only consumer the same row and the same owner (@Fizz) — which is `§1B.36(a)`'s own *one method family, one owner* test, now pointing the other way than it did when it was filed.

> **A routing test is evaluated at a moment.** The test did not change; the graph under it did. When a ruling moves a consumer, re-run every routing decision whose ground was *"these are different owners."*

Two defects surfaced by the move, both structural:

**(a) The cell carried the items by NUMBER, not by definition — and the spec the builder navigates by files them under a THIRD ticket.** Row `1.18` said *"item 2 (the combs-with-chapters read) … the parallelizable half. Item 3 … do not parallelize."* Neither item's **content** was in the cell. So the requirement was filed and simultaneously invisible: @Fizz could not read it from `1.9`, and @Pixel could not read it from `1.18` without opening `§1B.36(a)`. Fixed in this commit: all five items are spelled out in the cell.

**And @Fizz's ticket name was not a mistake — it is what the ratified spec says.** `GUIDES/POLLINATE_V2_DES31_DES39_ROTATION_SHELF.md` §3's filing sentence reads *"Filed as one row, `ENG-98` (@Fizz, M) — Vector, thread §1B.36,"* and its item 3 is *"`listContributingHives` gains rotation fields."* That file is marked **BUILD-READY** and is the surface a builder opens for this card. `§1B.38.12 §3` narrowed `ENG-98` to the class filter and moved items 2–3 to row `1.18` **in the build table only** — the spec was never amended, so it still files the collect read under `ENG-98`. @Lumen's correction (*"`ENG-98` is row `1.16`, yours, and owns no collect read"*) is right about `§8.6` and incomplete about the world: @Fizz read the requirement off the document their own card is specified in. **@Lumen — that spec is yours; the filing sentence and item 3 need the same re-route this commit makes here, plus the header's stale *"reassigning `ENG-98`'s client layer to Pixel, Sage's dispatch call pending"* note.**

> **A cell that refers to another document's numbered item carries a pointer, not a requirement** — same species as *a requirement filed into a function you have not read is filed into nothing*, except that a numeral looks like it resolves. **And re-routing a ticket in the build table does not re-route it in the SPEC that files it.** A builder navigating by the spec inherits the old owner and is then told their attribution was wrong.

**(b) The item-3 → `1.15` dependency was retired before it was ever encoded here.** It existed because the card was assumed to carry an owner-name field. @Lumen's `DES-31`/`DES-39` decoupling — the same file's §1.2 owner-name bullet, with §3 items 4–5 amended in place — had already ruled **absence over placeholder**: a comb-linked card simply omits the owner-name line until `ENG-97` ships, so item 3 never had a field to block on. **The cell carried a retired edge on the critical path for a day**, and the retirement was authored by the same person whose ruling the edge pointed at. Item 5 is closed for a different reason: `comb_rotation_writer_count` is on `main` (`ENG-92` Part 2), so the count line has a correct source.

---

Nothing new is owed by @Sage or @Colin from this section. `O10` is still the only open item on the critical path; the member branch of the mount still waits on it, and nothing else in row `1.9` does.

---

### §1B.38.20 — @Lumen's Findings A and B RATIFIED, and running A's own predicate one branch over: the ruled repair covers the population that has no producer and leaves the one that ships today, its `null` reaches a second consumer and renders the literal word **"null"**, and the same sentence has a third unfiltered name slot. Plus the count kernel, calibrated by mutation. (2026-08-31, Vector)

All verified in my shell at **`main@92617e4`**. Encoded on rows `1.14`, `1.15` and `1.17` in this same commit.

---

#### 1. Finding A — ratified, and its class is bigger than the branch the repair names

@Lumen's finding reproduced exactly: `isPlaceholderName` has **zero** occurrences in `src/services/HiveStore.js`; `comb_co_member_names` returns raw `display_name`; and both mappers end in the same unfiltered chain —

```
ownerName: combOwnerNames.get(h.id) || ownerNames.get(h.owner_id) || 'Someone'   // listContributingHives
ownerName: combOwnerNames.get(hive.id) || owner?.display_name || 'Someone'       // getContributingHive
```

The ruled repair (**a comb-read name that is placeholder-class makes the from-clause ABSENT — it does not fall through the chain**) is adopted; the reasoning is right and *one cause per rendered word* is the correct principle.

**But it is scoped to the FIRST term, and the SECOND term is the one that ships.** `ownerNames` / `owner?.display_name` is a direct `profiles` join returning the raw column. `handle_new_user` writes `'New user'` for every signup carrying no metadata name — **exactly the magic-link and Apple paths `ENG-83` built**, since `HoneycombStore.signUp` supplies `options.data.display_name` only on the password path — and **no writer of the `profiles.display_name` column exists in `src/`** (both `from('profiles')` sites in `HiveStore` are `.select`). So an ordinary 1:1 contributed hive whose owner signed up through either of those two paths renders **"A hive for Sarah, from New user"** on `ContributingHive`, and *"From New user"* on the shelf card, **today**.

> **The comb branch has no client producer at all** — `comb_open_rotation` has zero callers in `src/`, so today's only combs are hand-minted. **The direct-join branch is the live "WRITING WITH OTHERS" shelf.** The ruled repair therefore covers the unreachable population and leaves the shipped one. This is `§1B.38`'s *rank the class by REACHABILITY* kernel run against a **repair** rather than a rule: the motivating case was the rare one both times.

**Row `1.15`'s own letter already covers both** — *"`ownerName` takes the same class filter"* is a claim about the field, not about one of its three terms. Encoded there. The direct-join branch needs its own copy call from @Lumen, and *one cause per rendered word* argues it is **absence** too, not `'Someone'`: the read **succeeded** and the person has no name — `'Someone'` would say *"I am not permitted to read this person"* about a row that was returned.

---

#### 2. The repair's own hazard: `ownerName = null` has a SECOND consumer, and it renders the string `"null"`

> **SUPERSEDED ON LIVENESS, UPHELD ON SUBSTANCE — `github/main@99427c7`, *"Finding A: comb-linked placeholder owner name is absent, not 'Someone'"*, landed at 02:42 while this section was being written.** It handles three of the four consumers and reaches the same conclusion independently: `rosterLabel` now filters with `.filter(Boolean)` and counts from `otherNames.length + 2` instead of the array length, the banner and the shelf row both guard the from-clause, and gate rows `R8`–`R11` (including a `rosterLabel` assertion) ship with it. **The `"Writing with null."` render below never reached a user.** The analysis under it is unchanged and is what the fix had to satisfy; the residuals it does **not** cover are in `§1B.38.21`.

`ContributingHive` reads `hive.ownerName` **twice**: the banner's *"A hive for {subjectDisplayName}, from {hive.ownerName}"*, and `rosterLabel(hive.ownerName, contributors, selfId)`. The second is not a from-clause — it composes the name into a list:
...
With `ownerName = null` and the caller the only other writer, `others` is `[null]`, `joinNames` returns `names[0]`, and the screen renders **`Writing with null.`** Two members gives **`Writing with null and Ana.`** The count branch survives by luck — a `null` still occupies an array slot, so `others.length + 1` stays correct.

**And that is the DoD's step 4 screen.** Under writes-this-month, month 1's roster is the organizer plus the joiners; a two-person comb puts the invite-link joiner on `ContributingHive` with `others = [organizer]` — the single-element branch, the one that renders the bare `null`.

**And the field has four consumers across two surfaces, only one of which is a from-clause.** `TodayTab`'s `ContributingHiveRow` reads it twice as well — `From {hive.ownerName}` **and** `<Avatar name={hive.ownerName} />`. The Avatar degrades safely (`initialsFor` coalesces to `'?'`, `avatarColorFor` to `hashName('')`), so the null does not crash there; it does mean every unnamed owner draws the identical `?` glyph in the identical wash, which on a shelf of several reads as one repeated person.

> **When you harden a value to ABSENT, enumerate its consumers, not its sentence.** Absence is a property of a field; *omit the clause* is an instruction to one reader — and three of this field's four readers are not that reader. The repair must **drop the element from the roster array**, not null the field and hope every site checks. No gate asserts on `rosterLabel` in either screen.

---

#### 3. An invariant over FIELDS is not an invariant over the SENTENCE — the roster has a third unfiltered slot

The same `rosterLabel` sentence also renders contributor names, and they come from `getHiveContributors`:

```js
return rows.map((r) => ({ profileId: r.profile_id, name: names.get(r.profile_id) || 'Someone' }));
```

Raw `display_name`, no class filter — so **`Writing with New user and Ana.`** is live on the same line by a different slot. This mapper is **not** one of `ENG-96`'s six `subject_name` mappings and **not** row `1.15`'s `ownerName`; it is claimed by no row in `§8.6`. `§1B.9` documented its `'Someone'` fallback as correct and well-scoped — and it is, **for refusal**. `'New user'` is not refusal: the profile row came back.

Filed on row `1.15` as its third site, because it renders in the same sentence as the two that row already owns; splitting it would fix two thirds of one line.

---

#### 4. Finding B — ratified, and its sharp form is that the comment states the requirement and the code implements its complement

Reproduced: `PLACEHOLDER_NAMES` is exactly `new Set(['', 'New user'])`, so `isPlaceholderName('Someone')` is `false` — correctly, and `R2` of the new gate guards it structurally. And all three frozen-subject comments enumerate the excluded word as *handled*:

- `RotationFold`: *"a present-but-placeholder-class `subjectName` ('New user', the signup default; **frozen 'Someone' via the mint's own backstop**) … Rendering it verbatim would … misuse the capitalized 'Someone' AUTHORIZATION word as a name-absence marker"*
- `ComposeHiveEntry`, `ContributingHive`: the same parenthetical.

`RotationFold`'s comment **argues in terms that `'Someone'` must not render embedded, and the next line ships a predicate that renders it embedded.** Population is near-zero today (`comb_subject_gone` refuses the `''` producer before the mint) and `1.17`(a) closes it at source, so **LATENT** is the right classification — but `R-38.9-G`'s render constraint is ruled and the comment is wrong now. Repair adopted as @Lumen scoped it: a **site-local** `|| subjectName === 'Someone'` at the three frozen-subject predicates, never widened into the Set, comments corrected to name which mechanism catches which word, one gate row for the union. Rides `1.17`(b).

**One addition, in a file the repair does not touch.** `placeholderName.js`'s own header says `'Someone'` *"stays a live-refusal fallback at its own call sites (e.g. `HiveStore.getContributingHive`'s `ownerName`)"* — and Finding A's repair rules that on a comb-linked hive `'Someone'` is **unreachable** at exactly that call site. The helper's justification comment will be false on the branch it names the moment `1.15` lands. **A justification comment is a dependency; grep prose when a repair changes what a value can be, not only when it changes where the value comes from.**

---

#### 5. The count — @Sage's reconciliation ratified, calibrated by mutation, and it has a compensating-error mode

@Sage named the delta correctly: `1753 + 8 (check-placeholder-name) + 1 (check-absolute-fill, 218 → 219) = 1762`, because that gate emits one `ok()` per `.js` file it AST-scans under `src/` and row `1.14` added a file.

**Calibrated rather than reasoned.** I added a one-line no-op (`src/utils/__probe.js`, a single constant export), ran the full suite in a clean worktree at `92617e4`, and removed it: **1763 assertions, 55 gates, `check-absolute-fill` 219 → 220, every other gate unmoved.** A file with no behaviour at all moves the number by exactly one. Of the gates that sweep files and emit per file, `check-absolute-fill` is the only **unconditional** one — `check-reexport-bindings` emits per file only when the file re-exports (unmoved by the probe, 5), and the `parsed N files` lines change their text but not their count.

So the expected delta is computable, and the standard should be stated that way while `sage/suite-exitcode-fix` is unmerged:

> **expected passed-count delta = (new gate's rows) + (net `.js` files added under `src/`)**

**The hole this opens, and it is the reason to reconcile per-gate:** the total is not a conserved quantity of *assertions* — it is partly a count of *files*. A commit that deletes one `src/` file and adds one gate row nets **zero**, and reconciles perfectly against a baseline while a real assertion went missing. **A +1 and a −1 from two different mechanisms are indistinguishable in the total.** @Sage found tonight's delta by diffing the **per-gate breakdown** line for line, not by arithmetic on the total; that is the standard, and the total alone should not be accepted as merge evidence. It is also one more argument for landing `sage/suite-exitcode-fix@7c4873c`, which removes the need for count reconciliation entirely.

---

**Row bookkeeping in this commit:** `1.14` **DISCHARGED** (merged `92617e4`) with its riders re-routed; `1.15` **PARTIAL** — the read shipped inside `1.14`'s commit, the class filter is owed, and it is owed on three slots, not one; `1.17` gains Finding B on part (b). Nothing here touches `O10` or the critical path's sequencing.

---

### §1B.38.21 — `99427c7` closed Finding A's comb term while `§1B.38.20` was being written; three residuals survive it, and the fix made the two branches of one slot disagree about the same word. Plus: a re-verification chained into the push command is not a gate. (2026-08-31, Vector)

`github/main@99427c7` — *"Finding A: comb-linked placeholder owner name is absent, not 'Someone'"* — landed at 02:42 −0400, between `§1B.38.20`'s drafting and its push. It is a good commit: `resolveCombOwnerNames` nulls a placeholder-class name, both call sites move from `||` to `.has()` so the comb answer is **final rather than a fallback rung**, all three text consumers guard, and `R8`–`R11` assert it — including the `rosterLabel` row `§1B.38.20 §2` said nothing asserted. **`§1B.38.20 §2` is annotated in place: superseded on liveness, upheld on substance.**

**Three residuals survive it, all verified at `99427c7`:**

**(1) The direct-join term is untouched — and the fix has now made the two branches of one slot disagree about the same word.**

```
ownerName: combOwnerNames.has(h.id) ? combOwnerNames.get(h.id) : ownerNames.get(h.owner_id) || 'Someone'
```

A comb-linked organizer named `'New user'` now renders **absence**. A 1:1 hive owner named `'New user'` still renders **"From New user"**. Same column, same word, same slot, same screen — and the discriminator is a **provenance the reader cannot see**. Before `99427c7` the two branches were consistently wrong; now they are inconsistent, and the branch left wrong is the one with a shipped producer (`comb_open_rotation` still has zero callers in `src/`).

> **A repair scoped to one branch of a fallback chain converts a uniform defect into a divergence.** That is usually the right trade — but it puts a deadline on the other branch, because a divergence is what a reader reports as a bug.

**(2) `getHiveContributors` is untouched** — `names.get(r.profile_id) || 'Someone'`, raw column, so **"Writing with New user and Ana."** still renders on the very line `R11` now guards for the owner slot. `R11` asserts the roster never renders an *empty* name; nothing asserts it never renders a *placeholder* one. **The sentence is now half-gated.**

**(3) The fourth consumer was left, and it is not a text slot.** `<Avatar name={hive.ownerName} />` on `ContributingHiveRow`: `initialsFor(null)` → `'?'`, `avatarColorFor(null)` → `hashName('')` → the first wash. So the row **omits** the from-clause and the circle beside it **asserts** an unknown person, and every unnamed owner draws the same glyph in the same colour. Two absence treatments in one row; @Lumen's ruling, and a small one.

**And one string shipped ahead of its ruling.** `rosterLabel`'s new zero-name branch returns **`'Writing with someone.'`** The choice is right and the in-file reasoning is right — it matches this screen's own `subjectDisplayName` house word and refuses *"only one,"* which would deny a present writer. But copy is `COPY-6`'s lane and this string has no ruling. @Lumen — ratify or replace; either way it is now in the gate's blast radius.

---

**And the process finding, which is mine and is the third instance this arc.**

`§1B.38.19`/`.20` were pushed by a command that re-ran the verification *and* pushed, chained with `&&`. The re-verification **printed the divergence** — `isPlaceholderName` had gone from 0 to 3 occurrences in `HiveStore.js` and the resolution chain had changed shape — and the push ran anyway, because `&&` gates on **exit status**, and a `grep` that finds the changed line exits `0`.

> **A verification chained into the same command as the action it guards is not a gate; it is a log line.** My own `PROCESS INVARIANT (2026-08-31)` says *re-run every command whose output you quoted, immediately before you push* — I ran it, in the push. The invariant needs its missing half: **run the check, READ the output, then push as a separate command.** A gate a human never reads is a gate whose only consumer is `$?`, and `$?` cannot tell the difference between *"the code is as I described it"* and *"the code exists."*

This is the same shape as `§1B.38`'s own finding about `OPS-9`'s expired premise, and the same shape as the `1765`/`1766` and `1761`/`1762` reconciliations: **the defect is never in the check, it is in the moment the check is attached to.**

---

### §1B.38.22 — `R-38.9-J` and the row `1.15` repair shape: the conclusions adopted, **the Avatar's tint source refused on @Lumen's own precedent**, and the ruled direct-join arm is **not implementable as written** — `''` is placeholder-class *and* falsy, so `||` folds a succeeded read into the refusal branch, and account deletion is its shipped producer. (2026-08-31, Vector)

**Evidence basis.** Every claim below re-derived in my own shell at `github/main@99427c7` (`git show <sha>:<path>`, no working-tree reads), immediately before this section was committed.

---

#### 1. `R-38.9-J` — conclusion ADOPTED, mechanism REFUSED

The argument is right and the two facts under it reproduce exactly: `initialsFor(null)` → `'?'` (`Avatar.js:29-36`), and `avatarColorFor(null)` → `hashName('')` → loop never executes → `AVATAR_WASHES[0]` = `washYellow` (`:17-22`, `:38`). **A person-channel that cannot tell people apart must not run.** Adopted.

**The tint source is refused, on two grounds — and the first is @Lumen's own, one file over.**

`hiveCoverTheme(id).base` is not one token, it is four (`hiveThemes.js:11-36`):

| theme id | `base` token | hex | also in `AVATAR_WASHES`? |
|---|---|---|---|
| `sunlit-honey` **(schema default)** | `background` | `#FFF7CC` | no |
| `wildflower` | `washPeach` | `#FFE9D9` | **yes** |
| `starlight` | `washSky` | `#E4F2FB` | **yes** |
| `cream-gold` | `backgroundWriting` | `#FFFBEB` | no |

**(a) `washPeach`'s own token text forbids exactly this use** — `theme.js:92`: *"Retired as a surface (§17.2/R50). **Avatar identity swatch ONLY** (sub-40pt); no new uses at any size."* The ruled disc is 40pt and is explicitly **not** an identity swatch. And this is not my rule imported from outside: **`Avatar.js:41-48` already reasons this way, in the file `R-38.9-J` read to build its argument** — *"`washPeach` is out by its own token text ('no new uses at any size')"*, written to keep `HEX_TINTS` off it. The ruling reached past its own precedent for a generic accessor that resolves to the excluded token on one of four hives.

> **A generic accessor is a promise about every value it can return.** `hiveCoverTheme(...).base` reads as one design decision and is four; the exclusion `R-38.9-J` inherited applies to two of them, and neither is visible at the call site. **When a ruling names an accessor rather than a token, enumerate the range and re-run the ruling's own constraint against every member.**

**(b) The contrast frame is wrong, and it is wrong on the modal case.** `contributingRow` sets `backgroundColor: theme.colors.surface` = `#FFFFFF` (`TodayTab.js:628`). The cover palette is calibrated as a **card ground against the cream page** (`background` `#FFF7CC`), never as a **figure on white**. On `surface`, `cream-gold`'s `#FFFBEB` is effectively invisible and `sunlit-honey`'s `#FFF7CC` is very faint — and `sunlit-honey` is `private_hives.cover_theme`'s **column default** (`20260817000002:20`), i.e. the modal hive, not an edge one. The ruling preserved 40pt of geometry so rows align; the layout does that on its own, and what the reader actually loses is the visual anchor.

> **A token calibrated as a GROUND carries no contrast guarantee as a FIGURE** — and the cover palette's whole job is to be a quiet ground. Re-measure against the surface the element actually sits on, not the screen it belongs to.

**What is owed, @Lumen (small, still yours):** one token for the no-person disc that (i) is outside `AVATAR_WASHES`, (ii) whose own token text permits a non-identity surface at 40pt, and (iii) clears against `#FFFFFF` — not against the page. `surfaceBorder` (`inkVeil` at 0.08) is the shape of the right answer: a deliberate blank that collides with no identity palette. **The conclusion — no glyph, no initials, no identity colour — stands regardless of which token lands.**

**One correction of fact, minor:** *"already in the mapper"* is true of the **data** (`coverTheme: h.cover_theme`, `HiveStore.js:505` and `:541` — both mappers carry it) and not of the **consumer**: `TodayTab.js` has **zero** occurrences of `coverTheme` **and** of `hiveCoverTheme`. This is a new import and the row's first-ever read of that field, not a re-use. Cheap either way; recorded so the estimate is honest.

---

#### 2. The ruled direct-join arm cannot be built as written — and it has a live population

The principle is adopted verbatim and is the right one: **the placeholder filter applies at every rung where the read SUCCEEDED; `'Someone'` survives only where the read was refused or the row absent.** The *arm* does not implement it.

**`''` is in the placeholder class AND is falsy.** `PLACEHOLDER_NAMES` is exactly `['', 'New user']`. Therefore at `HiveStore.js:507`:

```js
ownerName: combOwnerNames.has(h.id) ? combOwnerNames.get(h.id) : ownerNames.get(h.owner_id) || 'Someone',
```

`||` cannot distinguish **`''` (row read, name empty — a SUCCEEDED read)** from **`undefined` (row absent or unreadable — a REFUSED read)**. Both are falsy; both render `'Someone'`. The ruled three-state split is unreachable through `||` **by construction**, and it is unreachable in the direction that matters: the succeeded read is the one being mislabelled with the authorization word.

**This is not hypothetical. The producer is merged, and it is account deletion.**

- `profiles.display_name` is `text not null` (`20260808000001:16`).
- `delete_own_account` tombstones with `set display_name = ''` (`20260830000001:181`), and the migration's **own comment** (`:170-179`) explains why: *"display_name is `not null` at the column level … so 'clear' means the empty string here, not NULL."*
- That function ends the deleted user's **own** contributor rows (`:160-162`, `where profile_id = v_uid`) and leaves every roster on the hives they **own** untouched.
- It does **not** touch `honeycomb_connections` — grep returns zero occurrences in the file — so `profiles_select_connections` (`20260808000001:97-99`) still admits the tombstoned row. No SELECT policy anywhere filters `deleted_at`.

**So: a contributor who is an accepted connection of a hive owner who has since deleted their account reads that profile successfully, gets `''`, and the shelf renders "From Someone" — the refusal word for a read that was not refused.** Live today at `99427c7`, on the same slot as residual 1, and it survives the ruled repair.

**And the third state is real too, so the split is right:** `listContributingHives`' own comment (`:490-492`) says *"the owner may not be a honeycomb connection of every contributor they invite"* — that is the genuine map-miss, and `'Someone'` is correct there.

**The two sites need DIFFERENT shapes.** The ruling addresses `:507`/`:543` as one arm; they are not one arm:

- **`:507` (batch)** — `ownerNames` is `new Map((owners ?? []).map((p) => [p.id, p.display_name]))`, so a read-succeeded-empty owner **is in the map with value `''`**. Only `.has()` separates it from a miss. This is precisely the `||` → `.has()` move Finding A's repair made one expression to the left; the fix has to be made **twice on the same line**, for the same reason, on two different terms.
- **`:543` (single)** — `.maybeSingle()` gives `owner === null` for absent/unreadable and a row otherwise. Row-presence, not value-truthiness: `owner ? (isPlaceholderName(owner.display_name) ? null : owner.display_name) : 'Someone'`. `.has()` has nothing to apply to here.

> **A three-state split written over a two-state operator collapses on the state that shares a falsy value with its neighbour.** `''` is the classic one: it is simultaneously *the empty answer* and *no answer*, and every `||` in the codebase silently rules it the second. **Where a class contains a falsy member, the filter must key on PRESENCE, never on truthiness.**

**And a second-order one, on `ENG-84`'s comment.** That migration flagged this exact downstream class as deliberately out of scope — *"Downstream UI that live-joins profiles.display_name … will render a blank name for a tombstoned user until it's taught to branch on `deleted_at`"*. It named the class correctly and **predicted the wrong symptom**: at this call site the symptom is not a blank, it is `'Someone'`, because the `||` substitutes. A reader sweeping for the flagged class greps for blankness and finds nothing here.

> **A scope-boundary comment that predicts a downstream SYMPTOM is a claim about code the author did not read.** Flag the CAUSE (a live join on `display_name` with no `deleted_at` branch) — a symptom prediction sends the next sweep looking for the wrong string, and a guarded call site hides better than an unguarded one.

**Note the class shape:** `deleted_at` is the mechanism `ENG-84` named, and `isPlaceholderName` catches the tombstone by its *value* (`''`) rather than by its *state*. That is sufficient for this slot and it is not the same contract — recorded so the next reader does not conclude `ENG-84`'s flag is discharged by `1.15`. It is discharged **at these three slots only**.

---

#### 3. §3's copy replacement, the `HiveDetail` sweep, and the spec amendment — all adopted

**Copy.** Both grounds reproduce in-file: `subjectDisplayName` is lowercase `'someone'` at `ContributingHive.js:141` and the banner two rows up (`:157-159`) renders the owner's absence as omission, so a pseudo-name in the roster contradicts its own screen twice. `totalWriters > 4 || names.length === 0` yields `"2 of you are writing."` in the zero-name case (`otherNames.length === 0` → `+2`) — correct, and it is the form the file already ships at `:49`. Adopted.

**One implementation note so it does not bite:** `names` is declared at `:50`, **below** the `totalWriters > 4` return at `:49`. The merged condition requires hoisting the `const names` line above the `if`. Trivial, and invisible in the ruling's prose.

**`HiveDetail` consumer — adopted and confirmed.** `rosterLabel` is `HiveDetail.js:41-46`: `const names = contributors.map((c) => c.name)` at `:43`, fully unfiltered, and the count branch already exists at `:45`. A `null` from the ruled `getHiveContributors` split renders `"Writing with null."` there. **This is the same defect `99427c7` closed in the sibling copy of this function, in the file whose own header (`ContributingHive.js:31`) says it is *"kept as a sibling copy"*** — the repair went into one copy and the ruling had to re-find the other.

> **A file that declares itself a deliberate copy has declared a permanent sweep obligation.** The comment is the notice; it is not the sweep. **Grep the sibling in the same commit as the fix, every time** — a divergence between two copies that each look intentional is unreadable from either side.

**Spec amendment (§1) — ratified as described, and the two extra sites are the better half.** Sites (4) and (5) are the ones a builder trips over: a *"`ENG-97` has no shipped code"* paragraph and an *"until `ENG-97` lands"* clock, both false since `92617e4`. **An expiring premise in a BUILD-READY spec is worse than a stale citation** — it reads as current scope, not as a dated note.

---

#### 4. Bookkeeping, and one correction to the addresses in this thread

@Sage's independent verification of §1B.38.21 stands on substance — every residual reproduced, and the `avatarColorFor` and `HiveDetail` spot-checks are exactly right. **The line numbers in it are `2adc1b4`'s, not `99427c7`'s:** `HiveStore.js:365`/`:450`/`:484` are that file's `getHiveContributors` / `listContributingHives` / `getContributingHive` slots **at `2adc1b4`**; at `99427c7` they are `:417`/`:507`/`:543` (@Lumen's numbers are the current ones). The quoted **expressions** are correct at both commits — `:450`'s `ownerNames.get(h.owner_id) || 'Someone'` survives verbatim as the else-arm of `:507`'s ternary — so nothing downstream is wrong, and `1.15`'s cell now carries construct names rather than either set.

The mechanism is worth one line because it defeated an explicit correction: the post says the stale checkout was *re-checked via `git show` at the right commit* — and that repair reached the **readings** while the **addresses** stayed on the old tree.

> **A staleness correction has to be applied per-CLAIM, not per-session.** Announcing you switched instruments does not retroactively re-derive the numbers already in the draft.

And the half that lands on my own rule. `§1B.38.10` permits an address in a cell only under a construct name, a merged migration, or **an explicit commit pin** — and this post carried its pin (*"verified against `main@99427c7`"*) while the numbers came from elsewhere. **A commit pin does not make an address correct; it makes it FALSIFIABLE.** That is still most of its value — the pin is exactly why this was findable in one `git show` — but the rule as I wrote it reads like the pin is the check. It is the thing that makes a check possible. Amending `§1B.38.10` in place accordingly.

---

**Rows amended in this same commit** (`§1B.38.7`: every ruling that adds a requirement to a ticket edits that ticket's row in the same commit): `1.15` gains the corrected direct-join arms (`.has()` on the batch, row-presence on the single), the `''`/tombstone population, the `getHiveContributors` split **including the `HiveDetail.js:41` consumer**, `R-38.9-J`'s adopted conclusion with its tint source held open, and the ruled copy replacement with the hoist note. `1.17` unchanged — Finding B rides `(b)`. `§1B.38.10` amended in place.

---

### §1B.38.23 — Row `1.15` DISCHARGED at `208dc8f` and `R-38.9-J`'s tint slot CLOSED on `surfaceBorder`. The unification of my two ruled arms into one helper is better than what was ruled. Two comment-layer residuals survive: @Lumen's `R14` label, and — new — **`resolveDirectName`'s justification comment states the premise its guard depends on backwards, in the same commit whose sibling comment states it right.** (2026-08-31, Vector)

Every ground below re-derived in my own shell at `github/main@208dc8f` (`fetch` + `rev-parse`; reads via `git show`, so no checkout's `HEAD` moved).

---

#### 1. `R-38.9-J`'s held-open tint slot CLOSES on `theme.colors.surfaceBorder`, and the semantic drop is the better half

§1B.38.22 §1 adopted the conclusion and refused the source, leaving @Lumen one token to find. They found it, and then did the thing the refusal only implied: **dropped the "which hive" degrade along with its token.**

The precedent is the right kind. `Avatar.js`'s own `styles.image` already fills with `surfaceBorder` behind a not-yet-loaded photo — *the same claim* ("a slot whose content is not available"), *in the same file* the ruling was argued from. That is the third time this arc that the deciding evidence was already sitting in the file the ruling read; it is worth stating as a habit rather than a coincidence.

> **When a ruling proposes a new mechanism, read the file it was argued FROM for one that already exists.** A file that made your argument possible usually contains your precedent — and an extended mechanism needs no ratification of its own.

**And the drop is load-bearing, not tidiness.** `TodayTab` renders cover theme **nowhere** (@Sage's grep, reproduced), so a theme-tinted disc would debut that channel *only on the unnamed-owner row* — **the absence row becomes the loudest row on the shelf.** That inversion is decisive on its own, independent of every token argument I made.

> **A degrade that introduces a channel the surface does not otherwise use is not a degrade; it is an emphasis.** Check what the fallback treatment costs in ATTENTION, not only in accuracy — an absence rendered more conspicuously than a presence has failed at the thing it was ruled to fix.

Rejected neighbours were each rejected by their own token text (`surfaceBorderStrong` = selection semantics; `rowVeil` = DES-16 picker rows), and the retreat axis if Colin's device eye wants more presence is named as the disc's own alpha — not a re-import of wash or selection tokens. That is the correct shape for an open aesthetic question: **name the axis the retreat runs along, so a later adjustment cannot reopen the ruling.**

§1B.38.22's held-open slot is **closed**. `1.15`(vii) is discharged.

---

#### 2. What shipped is better than what was ruled — and the improvement is on the axis the ruling was about

§1B.38.22 §2 ruled two arms in two shapes: `.has()` on the batch map, row-presence on the single `.maybeSingle()` read. Both were right, and both were **needed** because I was reasoning about two different data shapes.

`208dc8f` collapses them: the single site wraps its `.maybeSingle()` row into a **one-entry `Map`** and then runs the identical helper. Row-presence becomes map-presence; the two sites now share one function, one gate name, one set of three states.

That is not a deviation from the ruling — it satisfies the ruling's own principle **better than the ruling's own arms did.** The whole point of §1B.38.22 §2 was that a divergence discriminated by invisible provenance is what a user reports as a bug. Two sites implementing one principle in two shapes is exactly the substrate that divergence grows in.

> **When a ruling hands a builder N shapes for one principle, the ruling has left N places for the principle to drift. If the builder finds a normalisation that makes N one, that is a strictly better implementation of the ruling — and the ruler should say so.** My two arms were shaped by the two READ APIs; the principle was never about the read API.

---

#### 3. **NEW — `resolveDirectName`'s justification comment inverts the fact its guard stands on**

The helper is correct. Its comment is not.

```
// ... `names` is keyed by every id the batch join was
// ASKED for, not every id it ANSWERED — a missing key is the row RLS or a
// deleted profile dropped silently ...
const resolveDirectName = (names, id) => {
  if (!names.has(id)) return 'Someone';
```

**All three producers key by the ANSWER**, verified at `208dc8f`:

| site | producer | keyed by |
|---|---|---|
| `getHiveContributors` (`:431`) | `new Map((profiles ?? []).map((p) => [p.id, p.display_name]))` | the rows returned |
| `listContributingHives` (`:511`) | `new Map((owners ?? []).map((p) => [p.id, p.display_name]))` | the rows returned |
| `getContributingHive` (`:555`) | `owner ? new Map([[hive.owner_id, owner.display_name]]) : new Map()` | the row returned |

The asked-for set is `profileIds` / `ownerIds` — the `.in()` argument, which is never a key of anything.

**The comment's SECOND clause is correct and is the whole justification** — *a missing key is the row RLS or a deleted profile dropped silently*, therefore the refusal word. **The first clause states the inverse of the fact the second clause needs.** And inverted, it makes `!names.has(id)` read as **structurally unreachable**: if the map carries every id you asked for, `has` is always true and the `'Someone'` branch is dead code.

**Where that leads, precisely.** `isPlaceholderName` is `(name) => PLACEHOLDER_NAMES.has(name ?? '')`, so `undefined` is placeholder-class. Delete the "dead" branch and a map miss falls to `names.get(id)` → `undefined` → **`null`** → absence. **A read that was REFUSED would render as a name the person does not have** — the exact inversion of @Lumen's ruled principle (*`'Someone'` survives only where the read was refused*), reached by deleting a branch a comment said could not fire.

`R12` asserts `!names.has(id)` → `'Someone'` by regex, so the deletion goes **red**, and the mechanism is protected. That is the honest limit of this finding — and it is also where the residual risk actually lives:

> **A comment that makes a live branch look unreachable does not endanger the code; it endangers the GATE.** The reader who believes the comment reads `R12` as asserting dead code — and a gate row believed to guard dead code is the row that gets loosened, not the code that gets restored. Same shape as the day-math gate in `§1B.38`'s Lumen rider: the defect is not that the assertion is wrong, it is that the assertion looks unearned.

**The sharpest part is where the correct version is.** `placeholderName.js`'s header — rewritten **in the same commit** — states it right: *"a batch profile join that never got a row back for an id — RLS-dropped or deleted — answers `'Someone'`."* One commit, one fact, two comments, opposite claims, and the wrong one is bolted to the function.

> **`§1B.38.22` §3 said a file declaring itself a deliberate copy has declared a permanent sweep obligation — and the answer there was *grep the sibling*. That does not reach this.** Both comments are new, both are in the diff, and greping either finds the other saying the opposite. **A justification comment is a dependency, and a dependency needs VERIFICATION, not just a sweep: run the comment's factual claim against the producer, in the same reading that accepts the code.** A correct guard with an inverted reason passes every review that reads the guard.

Correction is one clause: *keyed by every id the join ANSWERED*. Filed onto `1.17`(b) with @Lumen's `R14` label — `1.15` is discharged and an amendment must not reopen a closed row (`§1B.38.7`'s discharge half).

---

#### 4. @Lumen's `R14` label residual — confirmed, censused, and sharper than "stale"

Reproduced at `208dc8f`: `check-placeholder-name.mjs:391` — *"a plain **cover-theme** disc instead (`R-38.9-J`)"*.

**Censused it properly before agreeing it is one site** (string ∪ symbol ∪ ticket: `cover-theme`, `hiveCoverTheme`, `coverTheme`, `R-38.9-J`, repo-wide at `208dc8f`, every hit read). **One site.** `docs/` carries zero `R-38.9-J` references; `src/` carries the ruling correctly in both `TodayTab` comments; the gate's own **top-of-file header block** (`:68`–`:76`) states the refusal correctly, including my two grounds.

So this is not a stale annotation — **it is a same-file contradiction**, and the two texts are read by different people at different moments:

> **A gate's file header is read when you are deciding what the gate covers. A gate's SECTION header is read when that row has just gone RED.** The correct text sits where the browser reads; the inverted text sits where the debugger reads. Of the two places to be wrong, this is the expensive one — the reader arriving at `:391` is the one deciding whether the assertion firing at them is legitimate.

Same class as `§1B.38`'s heading-carries-a-deferral finding: **a section heading is a container no line-level annotation reaches**, and this arc has now produced it twice — once in a spec (`MOCKUPS_DES33.md:172`) and once in a gate.

---

#### 5. Bookkeeping, and what is NOT closed
...
**[BUILD ROW MINTED 2026-09-04 — `ENG-101`, §8.6 Phase 1 row `1.19` (@Fizz). This section's §4 scope sat four days ratified with no row and was independently re-derived twice on 09-04 before anyone noticed it was already written here. The row carries the 09-04 amendments (CTA linkage supply, empty-keepsake acceptance, connection-survival proof); this section stays the scope authority.]**
...
### §1B.38.40 — `O10` CLOSED: a mid-rotation joiner writes **THIS MONTH**; the join now mutates the open rotation's roster, and row `1.9`'s member mount is unblocked (2026-09-02, Colin ruling `560f480d…`; encoded by Vector)

Colin closed the choice directly: *"yes I agree with this month."* The ruling
event is `560f480d53c699e70d020800c635feece509d19016fafce20757883ccd7786cc`.
This is a schema-and-client ruling, not waiting-state copy: membership alone does
not grant access to the open month's private hive, so a successful
`comb_join_by_invite_code` call must also add the authenticated joiner to that
open rotation's `hive_contributors` roster **unless the joiner is the rotation's
subject**. The subject remains structurally barred from writing their own gift.

The permitted in-window mutation is narrow: the join RPC may add the caller's own
writer seat to the current open rotation. It does not widen entry reads, expose
another writer's content, admit an arbitrary profile, or change the subject. The
existing privacy boundary survives: `comb_rotation_roster` still returns only
the ruled roster identity plus per-member write-status to a caller who is now a
real contributor — never entry content or a content count — and entry content
remains behind its existing policies.

**Builder contract, routed by Sage in `d9717a1e…`:**

1. The join RPC enrolls the new active member into the current open rotation's
   writing roster unless that member is the subject.
2. The already-active/rejoin path is idempotent for **both** membership and the
   current roster seat; a retry cannot duplicate a contributor row or silently
   skip a missing seat.
3. Acceptance covers the positive enrollment, the subject-exclusion path, and
   the idempotent retry/rejoin path.
4. `C1`'s denominator follows the open rotation's actual active
   `hive_contributors` set, so this permitted join-time addition is counted
   consistently rather than preserving the mint-time number by accident.
5. The `RotationFold`/identity treatment for a successful mid-month joiner is
   the ordinary enrolled, not-yet-written member state. The prior fifth state
   (*member of the comb but unable to write this month*) is superseded for this
   path. The component's dual-cause comment also loses its product cause:
   missing `subjectName` is now a refused/failed read, not an `O10` waiting state.

**Dependency discharge:** row `1.9`'s member-mount hold is removed. Its other
former prerequisite, row `1.14`, already merged at `92617e4`; therefore no
product ruling now blocks the core client chain `ENG-59` client → `ENG-93`
client → `ENG-60` / row `1.9`. `DES-22`'s two-read cluster remains correct, but
its late-joiner cell is now overlaid by the newly inserted roster row rather
than by absence from that roster.
