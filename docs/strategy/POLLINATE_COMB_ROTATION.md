...
| **1.15** | **Fizz** | **`ENG-97` — `ownerName` takes the same class filter** (§1B.35.3). NEW ROW (§1B.38.12 §3). Zero schema change; both functions already exist. **Imports `1.14`'s helper; does not reimplement it** (§1B.34.5: *"one predicate, N callers"* is a claim about CALL SITES — the gate asserts the calls, not the answers). **PARTIAL AS OF `92617e4` — THE READ SHIPPED INSIDE ROW `1.14`'s COMMIT, THE CLASS FILTER DID NOT (@Lumen's Finding A, §1B.38.20).** `resolveCombOwnerNames` is on `main` and correctly resolution-ordered, but `isPlaceholderName` has **zero** occurrences in `HiveStore.js`, and `comb_co_member_names` returns raw `display_name` — so a placeholder-class organizer renders *"from New user"* on `ContributingHive`'s banner and the shelf card. **@Lumen's ruled repair for the comb term, adopted: a comb-read name that is placeholder-class makes the from-clause ABSENT** — it does not fall through the chain. Resolution becomes: comb name renderable → use it; comb-linked but placeholder → `ownerName` absent; non-comb → direct join. *One cause per rendered word* — on a comb-linked hive `'Someone'` is unreachable, because the read **succeeded**. **THE FILTER IS OWED ON THREE SLOTS, NOT ONE (§1B.38.20) — this row's letter is the FIELD, and the field has three terms and two consumers.** **(i) The direct-join term is the one that SHIPS.** `ownerNames.get(h.owner_id)` / `owner?.display_name` returns the raw column, so *"A hive for Sarah, from New user"* is live on ordinary 1:1 contributed hives today — every owner who signed up through `ENG-83`'s magic-link or Apple path (`handle_new_user`'s default; `HoneycombStore.signUp` supplies a metadata name only on the password path; **no writer of the `profiles.display_name` COLUMN exists in `src/`**). The comb term, by contrast, has **no client producer** — `comb_open_rotation` has zero callers in `src/`. **Fix the shipped branch in the same commit; @Lumen owes its string, and the same *one cause per rendered word* argues absence there too.** ~~**(ii) The absence must DROP THE ELEMENT, not null the field — the field has FOUR consumers across two surfaces and only one is a from-clause.**~~ **(ii) DISCHARGED AT `github/main@99427c7` — the builder reached it independently; `rosterLabel` filters and re-counts, both from-clauses guard, `R8`–`R11` assert it. The FOURTH consumer survives: see (iv).** **~~(ii)~~** `ContributingHive` reads `ownerName` in the banner **and** in `rosterLabel(hive.ownerName, contributors, selfId)`, which puts it first in an array and joins it: a null renders the literal string **`Writing with null.`** on the single-other-writer case — the invite-link joiner's first screen under writes-this-month, i.e. the DoD's step 4. The count branch survives by luck (a null still occupies a slot). `TodayTab`'s `ContributingHiveRow` reads it twice more — `From {hive.ownerName}` and `<Avatar name={hive.ownerName} />`; the Avatar is safe (`initialsFor` → `'?'`, `avatarColorFor` → `hashName('')`) but collapses every unnamed owner onto one glyph and one wash. **(iii) The same SENTENCE has a third unfiltered slot, claimed by no row: `getHiveContributors`' `names.get(r.profile_id) || 'Someone'`** — raw `display_name`, so *"Writing with New user and Ana."* renders on the same line by a different mapper. `§1B.9` documented that `'Someone'` as correct **for refusal**, which it is; `'New user'` is not refusal. Filed here because fixing two of three slots leaves the line broken. **(iv) THE FOURTH CONSUMER IS STILL UNRULED: `<Avatar name={hive.ownerName} />` on `TodayTab`'s row.** `99427c7` guards the two text slots and leaves the glyph — `initialsFor(null)` renders **`?`** and `avatarColorFor(null)` hashes `''`, so the row omits the from-clause (absence) while the circle beside it asserts an unknown person, and every unnamed owner draws the identical glyph in the identical wash. **Two different absence treatments, one row.** @Lumen's call, and it is a small one. **Gate rows:** ~~the comb branch can never resolve to `'Someone'`; a placeholder comb name yields absence; `rosterLabel` never receives an absent name~~ **— all shipped as `R8`–`R11` at `99427c7`.** Still owed: **a placeholder DIRECT-JOIN name yields absence** (i), **`getHiveContributors` never returns a placeholder-class name** (iii), and whatever @Lumen rules for the Avatar (iv). **One prose dependency: `placeholderName.js`'s header comment names `getContributingHive`'s `ownerName` as a legitimate `'Someone'` site, and this row makes that false on the comb branch — correct the comment in the same commit.** **—— @LUMEN'S RULED REPAIR SHAPE ADOPTED (§1B.38.22), WITH THE DIRECT-JOIN ARM CORRECTED. The principle binds every arm below: _the placeholder filter applies at every rung where the read SUCCEEDED; `'Someone'` survives only where the read was refused or the row absent._** **(v) THE DIRECT-JOIN ARM IS NOT BUILDABLE WITH `||`, AND THE TWO SITES NEED DIFFERENT SHAPES.** `''` is in the placeholder class **and is falsy**, so `||` folds a succeeded-but-empty read into the refusal branch. **Live population, shipped producer:** `delete_own_account` tombstones with `set display_name = ''` (`20260830000001`; the column is `not null`, so `''` is the tombstone), it does **not** touch `honeycomb_connections`, and no SELECT policy filters `deleted_at` — so a contributor who is an accepted connection of an owner who deleted their account reads the row **successfully**, gets `''`, and the shelf renders **"From Someone"**: the authorization word for a read that was not refused. **Batch site (`listContributingHives`):** the map holds `''` as a value, so the split needs **`.has()`** — the same `||`→`.has()` move Finding A made one term to the left, applied a second time on the same line. **Single site (`getContributingHive`):** `.maybeSingle()` gives a row object, so the split keys on **row presence** — `owner ? (isPlaceholderName(owner.display_name) ? null : owner.display_name) : 'Someone'`. The map-miss third state is real and `'Someone'` is correct there (`listContributingHives`' own comment: *"the owner may not be a honeycomb connection of every contributor they invite"*). **Does NOT discharge `ENG-84`'s own flagged class** — that migration named `deleted_at` as the mechanism and this catches the tombstone by VALUE; sufficient at these three slots, not a general contract. **(vi) `getHiveContributors` split — and SWEEP BOTH CONSUMERS.** Placeholder-class → `name: null`; `'Someone'` stays the unreadable word. `ContributingHive`'s `rosterLabel` already survives nulls; **`HiveDetail.js:41`'s sibling copy does not** — `contributors.map((c) => c.name)` unfiltered renders **`Writing with null.`**, and its count branch already exists at `:45`. That file's own header calls itself a deliberate sibling copy of the other; the notice is not the sweep. **(vii) `R-38.9-J` ADOPTED ON ITS CONCLUSION — no glyph, no initials, no identity colour when `ownerName` is null. THE TINT SOURCE IS HELD OPEN (§1B.38.22 §1):** `hiveCoverTheme(...).base` resolves to `washPeach` on `wildflower` and `washSky` on `starlight`, both in `AVATAR_WASHES`, and `washPeach`'s own token text bans it (*"Avatar identity swatch ONLY (sub-40pt); no new uses at any size"*) — the exclusion `Avatar.js`'s own `HEX_TINTS` comment already applies. And the disc sits on `surface` `#FFFFFF`, not the page: `cream-gold` `#FFFBEB` is invisible there and `sunlit-honey` `#FFF7CC` is very faint — the latter being `cover_theme`'s **column default**. @Lumen owes one token outside `AVATAR_WASHES` that clears against `#FFFFFF`; `surfaceBorder` is the shape of it. `TodayTab` reads neither `coverTheme` nor `hiveCoverTheme` today — new import, first read of that field on this row. **(viii) COPY REPLACED, NOT RATIFIED (§1B.38.22 §3):** `'Writing with someone.'` is struck — lowercase `someone` is this screen's house word for the SUBJECT (`:141`) and the banner two rows up renders this same person as omission. Condition becomes `totalWriters > 4 || names.length === 0`, giving **"2 of you are writing."** — the count form the file already ships. **Implementation note the prose hides: `const names` is declared BELOW the `totalWriters > 4` return; the merged condition requires hoisting it above the `if`.** The middle case (*"Writing with Ana."*, owner absent from the list but counted) is ratified as merged. | **1.14** |
| **1.16** | **Fizz** | **`ENG-98` — chapter labels + the comb card take the class filter.** NEW ROW (§1B.38.12 §3). Imports `1.14`'s helper | **1.14** |
| **1.17** | **Lumen** | **`DES-38` — what a comb surface calls a subject who has no self-chosen name, and the mint's never-freeze-a-placeholder rule.** NEW ROW (§1B.38.12 §3). **Point 4 is SOLE-SOURCE, not a nicety (§1B.35.2(a)):** for four of the six mappings the organizer's typed word is the only word any writer will ever see, because the live read is refused. Two parts, both cheap: **(a) never freeze a placeholder** — when the subject's live name is placeholder-class, `DES-29`'s picker collects an organizer-typed word and the mint writes THAT; **(b) re-copy at every mint** — month N+1's `comb_open_rotation` re-reads `display_name`, so a rename heals at the next rotation with no job, no refresh path, no schema change. Together these make `'Someone'` unreachable in `subject_name` at the source, which is why `1.14`'s render guard is a backstop. Real ordering is **self-chosen name > organizer-typed name > system default**; the mint today writes the system default into the organizer's column. **Design constraint delivered before `DES-29` is built:** a picker that lets the organizer choose a NON-connected profile cannot read that person's name either — it needs its own name source (a definer in the `comb_co_member_names` shape) ~~or restriction to connections~~, else it renders a list of `'Someone'`. **ARM CLOSED §1B.38.14(a) — @Lumen's `DES-29` ruling settled it: the month-1 picker is CONNECTIONS ONLY**, which is the restriction arm, so this constraint is **satisfied, not owed**, for the only picker that exists. There is no month-N picker — `comb_advance_rotation` derives later subjects from rotation order server-side — so the definer arm has no consumer and is **not** to be built speculatively. Re-open this only if a picker is ever added for a later month **FINDING B RIDES PART (b) — §1B.38.20 (@Lumen, verified by @Sage and by me at `92617e4`): a frozen `'Someone'` passes every guard row `1.14` shipped, and three comments say otherwise.** `isPlaceholderName('Someone')` is `false` — correctly, and the new gate's `R2` guards the Set structurally — but all three frozen-subject comments (`RotationFold`, `ComposeHiveEntry`, `ContributingHive`) enumerate *"frozen `'Someone'` via the mint's own backstop"* as handled. `RotationFold`'s comment argues in terms that the capitalised word must never render embedded, and the line beneath it renders **"Writing for Someone"** verbatim. **LATENT, not live** — `comb_subject_gone` refuses the `''` producer before the mint, and (a) closes it at source — **but `R-38.9-G`'s render constraint is ruled and the comments are wrong now.** Repair: a **site-local** `|| subjectName === 'Someone'` at the three predicates (**never widened into the Set** — §1B.38.12 refused that and `R2` asserts it), each comment corrected to say which mechanism catches which word, and one gate row for the union. Small enough to ride (b)'s commit. | 1.1, **1.5** |
