...
>
> **New, unruled, flagged for Colin — Garden's dominant content is structurally near-empty for most of TestFlight month one, and the cause is this morning's own ratification.** Nectar has exactly two inflows, both re-verified against `github/main@b62ed2c`: a 500-drop one-time grant at consent (`nectar_starter_grant_drops()`), and a refill that fires only on a sealed rotation delivery (`nectar_refill_rotation_members`, called from `advance_due_rotations` only when `v_sealed`). There is no purchase path (19a). A comb delivers monthly, so in month one almost nobody has a delivery yet, the refill essentially never fires, and the one-time grant is the entire supply — Garden's nectar band renders "nothing received, from nobody" for most testers, on the last of five tabs. `RecapTab.js`'s streaks and dominant themes, by contrast, are computed from the user's own entries and populate for anyone who has written twice. **Recommendation (Vector's, unruled): lead Garden with the streaks/themes band and treat received-nectar as the second band, designed for empty rather than assumed full** — same eventual tab, reordered for the window where TestFlight can actually teach us something. This is Colin's call, not decided here.
>
