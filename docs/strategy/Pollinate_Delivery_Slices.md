...
>
> **Phases 0, 1 and 2 of `POLLINATE_COMB_ROTATION.md` §8.6 are the MVP. Build to completion now.** Named **MVP-Comb** so it is not confused with the narrower "MVP1" the amendments below define. Full scope, exclusions and definition-of-done: that document's **§1A**.
>
> **In:** the rotation engine (`ENG-58`/`85`/`83`/`59`/`60`, `OPS-9`, `DES-21`/`22`/`29`/`31`, `COPY-6`), the daily short-note + simulated-nectar layer (`ENG-62`/`90`/`65`/`66`, `DES-23`/`32`, `COPY-7`), and Phase 0's unblockers (`OPS-8`, `ENG-84`, `COPY-13`, `OPS-7`, `OPS-3`). The approved merge queue, the `MVP1_DEMO_READINESS_AUDIT.md` demo-gap items and the GL1/GL2 luxury pass all **carry in — none of it is cancelled.**
>
> **Out:** Phase 4 entirely (IAP, paywall surfaces, and **cap enforcement — the caps are built but disabled**, §8.5), and all of 19b/c/d (real sats, Breez/Spark, Privy, `LEGAL-1`). Nectar is **simulated only.**
>
